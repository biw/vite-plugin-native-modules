...
const getChecks = (repo, sha, patterns) => {
  ...
  const checkRuns = (checkRunsResponse.check_runs || []).map((check) => {
    const text = [check.name, check.app?.name, check.app?.slug, check.details_url, check.html_url]
      .filter(Boolean)
      .join(" ");
...
const classifyFreshness = (comment, headSha, sinceMs, includeStale) => {
  ...
  if (!comment.commitId && sinceMs !== undefined) {
    ...
    if (createdAt !== undefined && createdAt < sinceMs) {
      return {
        fresh: false,
        staleReason: `top-level comment predates ${new Date(sinceMs).toISOString()}`,
      };
    }
  }
...
const collectBotComments = (repo, pull, options, sinceIso) => {
  ...
  const inlineComments = fetchPagedArray(
    `/repos/${repo.owner}/${repo.name}/pulls/${pull.number}/comments`,
  );
  const reviews = fetchPagedArray(`/repos/${repo.owner}/${repo.name}/pulls/${pull.number}/reviews`);
  const issueComments = fetchPagedArray(
    `/repos/${repo.owner}/${repo.name}/issues/${pull.number}/comments`,
  );
  const freshComments = [];
  const staleComments = [];
  ...
  return {
    freshComments: freshComments.sort((a, b) =>
      String(a.createdAt).localeCompare(String(b.createdAt)),
    ),
    staleComments: staleComments.sort((a, b) =>
      String(a.createdAt).localeCompare(String(b.createdAt)),
    ),
  };
};
...
const buildSnapshot = async (options) => {
  ...
  const checks =
    waitResult.checks.length > 0
      ? waitResult.checks
      : getChecks(repo, pull.headSha, options.checkPatterns);
...
const renderMarkdown = (snapshot) => {
  ...
  for (const comment of snapshot.comments.staleComments) {
    const location = comment.path
      ...
      : "top-level";
    lines.push(
      `- ${comment.author} ${comment.type} at ${location}: ${comment.staleReason} (${comment.htmlUrl})`,
    );
  }
